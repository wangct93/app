...
forEach:function(_6,_7,_8){ //TODO forEach
//var _9=[];
//for(var i=0;i<_6.length;i++){
//_9.push(_6[i]);
//}
for(var i =0,len=_6.length;i<len;i++){
var _a=_6[i];
if(_8(_a)==false){
return true;
}
if(_7&&_a.children && _a.children.length > 0){
	if(this.forEach(_a.children, _7, _8)){
		return true;
	}
...
if(_99.selected) {
	c += _99.plain ? "l-btn-selected l-btn-plain-selected" : "l-btn-selected";
}
var _9a = "<span class=\"l-btn-left";
var btnText = "";
if(_99.text) {
	btnText += '<span class=\"l-btn-text\">'+_99.text+'</span>' ;
} else {
	btnText += '<span class=\"l-btn-text l-btn-empty\">&nbsp;</span>' ;
}
if(_99.iconCls) {
	btnText += '<span class=\"l-btn-icon '+_99.iconCls+'\">&nbsp;</span>';
	_9a += " l-btn-icon-" + _99.iconAlign;
}
_9a += "\">"+btnText+"</span>";

t.removeClass("l-btn-plain l-btn-selected l-btn-plain-selected l-btn-outline")
	.removeClass("l-btn-small l-btn-medium l-btn-large").addClass(c).attr("group", _99.group || "")
	.attr("id", _99.id || "")
	.unbind(".linkbutton").bind("focus.linkbutton", function() {
		if(!_99.disabled) {
...
(function($){
function _ab(_ac){ //创建分页
	var _ad = $.data(_ac, "pagination");
	var _ae = _ad.options;
	var bb = _ad.bb = {};
	var _af = $(_ac).addClass("pagination");
	var table = $("<table cellspacing=\"0\" cellpadding=\"0\" border=\"0\"><tr></tr></table>");
		...
		switch (_b2) {
			case "list":
				...
				for(var i = 0; i < _ae.pageList.length; i++) {
					options[options.length] = "<option>" + _ae.pageList[i] + "</option>";
				}				
				list.push('<td><select data-p='+_b2+' class=\"pagination-page-list\">') ;
				list.push(options.join("")) ;
				list.push('</select></td>') ;
				break ;
			case "sep":
				list.push('<td data-p='+_b2+'><div class=\"pagination-btn-separator\"></div></td>');
				break ;
			case "first":
			case "prev":
			case "next":
			case "last":
			case "refresh":
				list.push('<td><a data-p='+_b2+' href=\"javascript:void(0)\"></a></td>') ;
				break ;
			case "links":
				list.push('<td data-p='+_b2+' class=\"pagination-links\"></td>');
				break;
			case "manual":
				list.push('<td><span style=\"padding-left:6px;\">'+_ae.beforePageText+'</span></td>');
				list.push('<td><input data-p="num" class=\"pagination-num\" type=\"text\" value=\"1\" size=\"2\"></td>');
				list.push('<td><span data-p="after" style=\"padding-right:6px;\"></span></td>') ;
				break;
			default:
				break;
		}
	...
	tr.find('[data-p]').each(function(i,item){
		var h = $(item);
		var dataP = h.attr('data-p') ;
		var btn = _ae.nav[dataP];
		switch(dataP){
			case "list":
			...
			case "num":
				bb[dataP] = h;
				h.unbind(".pagination").bind("keydown.pagination",function(e){
					if(e.keyCode==13){
	...
	_af.html(table).append("<div class=\"pagination-info\"></div><div style=\"clear:both;\"></div>");
...
$.fn.pagination=function(_ca,_cb){ //分页
...
$.data(_125,'nodeCache',createCache(data));
...
//生成缓存
function createCache(datas){
	var cache = {};
	addCache(datas, cache);
	return cache;
}

//添加缓存
function addCache(datas,cache){
	for(var i = 0 ,len = datas.length;i<len;i++){
		var data = datas[i] ;
		data.target=$("#"+data.domId)[0];
		cache[data.domId] = data ;
		if(data.children){
			addCache(data.children,cache) ;
		}
	}
} 
...
function _ea(_180,_181){//TODO 11
return _12b(_180,"domId",$(_181).attr("id"));
};
function _182(_183,id){
return _12b(_183,"id",id);
};
function _12b(_184,_185,_186){// TODO 22
	var _187 = null;
	var c = $.data(_184,'nodeCache') ;
	if(c){
		if(c[_186]){
			_187 =  c[_186];
		}
...
function _188(_189,_18a){//TODO tree的select
...
$(_263).find("div.panel,div.accordion,div.tabs-container,div.layout,.easyui-fluid").filter(function(_265,el){
if($(this).is(":visible")){//将visible判断放到过滤中做，提高选择器效率
...
var p_opt = p.panel("options");
if(p_opt.collapsible==_2c5){
var h=isNaN(_2c6)?undefined:(_2c6+_2c1*h.length);
if(p_opt.panelHeightAuto){		/********添加panel高度适应内容高度判断**************/
	var $lis = p.children().children();
	h = $lis.length * parseInt($lis.first().children().css('height')) + p.prev().height();
}
p.panel("resize",{width:cc.width(),height:(_2c5?h:undefined)});
...
var _4a9=/([a-zA-Z_0-9]+)(.*)/.exec(_4a6);
var rule=opts.rules[_4a9[1]];
if(rule&&(_4a8||_4a6=="finiteRequired")){//有限非空验证不需要输入有值
var _4aa=_4a7||opts.validParams||eval(_4a9[2]);

//TODO /**************对验证框架进行改造******************/
var result = rule["validator"].call(_4a0,_4a8,_4aa);
if(result == false){
...
$(this).tooltip("tip").addClass("invalid-color");//验证失败提示颜色
$(this).parent().addClass("valid-invalid");
},onHide:function(){
$(this).tooltip("destroy");
...
$("<input type=\""+opts.type+"\" "+(opts.unselectable?" unselectable=\"on\" " : "")
		+" class=\"textbox-text\" autocomplete=\"off\">").prependTo(tb);
}
...
if(bb.length){
var bc = $("<span class=\"textbox-addon\"></span>").addClass("textbox-addon-"+opts.iconAlign);	
...
.addClass("textbox-button-"+opts.buttonAlign).linkbutton({text:opts.buttonText,iconCls:opts.buttonIcon});
btn.prependTo(tb);
}
_4c7(_4c5); //TODO 绑定验证
_4c8(_4c5,opts.disabled);//设置disabled
_4c9(_4c5,opts.readonly);//设置readonly
};
function _4ca(_4cb){
...
tb._size(opts,_4d0);
var h = tb.height(),
	_4cdStyle=_4cd.style;
btn.linkbutton("resize",{height:h});
...
_4d4.css({width:opts.iconWidth+"px",height:h+"px"});
var css = {
		paddingLeft:(_4cdStyle.paddingLeft||""),
		paddingRight:(_4cdStyle.paddingRight||""),
		marginLeft:_4d5("left"),
		marginRight:_4d5("right")}; 

if(opts.multiline){
	css.paddingTop = (_4cdStyle.paddingTop||"");
	css.paddingBottom = (_4cdStyle.paddingBottom||"");
_4d2._outerHeight(h);
}else{
	css.paddingTop = 0;
	css.paddingBottom = 0;
	css.height = h+"px";
	css.lineHeight = h+"px";
//_4d2.css({paddingTop:0,paddingBottom:0,height:h+"px",lineHeight:h+"px"});
}
...
var tg = $(_5a2);
var _5a6=tg.addClass("spinner-f").textbox($.extend({},opts,{icons:_5a4})).textbox("getIcon",_5a4.length-1);
_5a6.append("<a href=\"javascript:void(0)\" class=\"spinner-arrow-up\" tabindex=\"-1\"></a><a href=\"javascript:void(0)\" class=\"spinner-arrow-down\" tabindex=\"-1\"></a>");
tg.attr("spinnerName",tg.attr("textboxName"));
_5a3.spinner=tg.next();
...
for(var i=0;i<trs2.length;i++){//TODO 影响加载速度
	break;
//var tr1=$(trs1[i]);
//var tr2=$(trs2[i]);
//tr1.css("height","");
...
function _62f(_634,_635,_636){ //创建表头_634：header；_635：fields；_636
	if(!_635){
		return;
	}
			...
			if(col.checkbox){
				...
				if(col.field){
					...
					if(col.width){
						...
						if(col.fixed == false){//TODO 如果是自适应，最小宽度为设置的宽度
							cell.css("minWidth",col.width);
						}
					}else{
						col.auto=true;
					}
	...
	for(var i=0;i<_639.length;i++){
		_672(_62a,_639[i],-1); //设置colspan
	}
	$(_634).empty().append(t).show();
};
function _630(){
var _63b=[];
...
function _672(_673,_674,_675){//设置coslpan。_673:table；_674：fields,_675：colspan差值
_676(true);
_676(false);
function _676(_677){
...
if(!col.minWidth){ //TODO
	col.minWidth = col.width;
}
if(_686(col)){
_684+=col.width;//TODO 自适应列宽
...
function _6a1(_6a6,tr){
var dc=$.data(_6a6,"datagrid").dc;
var p = tr;
if(!tr){
	p = dc.view;
}
p.find("div.datagrid-editable").each(function(){
var cell=$(this);
...
function _678(_6ab,_6ac){//获取表头二维数组，即根据配置的columns、frozenColumns按照colspan和rowspan整理成一个二维数组。
var opts=$.data(_6ab,"datagrid").options;
var _6ad=_6ac?opts.frozenColumns:opts.columns;
var aa=[];
var _6ae=_6af();//表格的colspan总数
...
var table = $("<table border=\"0\" cellspacing=\"0\" cellpadding=\"1\"><tr><td></td></tr></table>")
			.bind("click dblclick contextmenu",function(e){
				e.stopPropagation();
			});
cell.empty().append(table);
$.data(cell[0],"datagrid.editor",{actions:_71e,target:_71e.init(table.find("td"),_71d),field:_71b,type:_71c,oldHtml:_71f});
}
...
function _670(_753,_754,cb){ //加载数据
...
_629(this);//创建表头、创建工具条、分页
_5f6(this);//创建数据表格panel
_63f(this);//绑定事件
if(opts.data){
...
opts.view.renderEmptyRow(this); //没有数据创建EmptyRow
...
$(_7bc).html(_7bf.join(""));
},renderTable:function(_7c0,_7c1,rows,_7c2){//创建表格数据
...
return _7c5.join("");
},renderRow:function(_7c8,_7c9,_7ca,_7cb,_7cc){//创建行数据
...
if(col.fixed == false){//TODO 最小宽度
	_7d0+="min-width:"+col.minWidth+"px;";
}
...
if(col.formatter){
//TODO 收集字段格式化后的html
var ft = col.formatter(_7cf,_7cc,_7cb); //value,row,index
if(!window.formatterCache){
	window.formatterCache = [];
}
var ftc = window.formatterCache;
if(!ftc[_7cb]){
	ftc[_7cb] = {};
}
var fName = col.fieldName || col.field;
ftc[_7cb][fName] = ft;
...
var tableName = opts.dtableName;
top.requestCache[tableName] = $.ajax({type:opts.method,url:opts.url,data:_7ff,dataType:"json",success:function(data){
_800(data);
},error:function(){
...
var name=$(_9c1).attr("textboxName")||""; //TODO 修改combo的值的生成方法
var arr = "";
for(var i=0;i<_9c2.length;i++){
	arr = arr + "<input type=\"hidden\" class=\"textbox-value\" name='"+name+"' value='"+_9c2[i]+"' ";
	if(opts.disabled){
		arr = arr +  "disabled='disabled' ";
	}
	arr = arr +  " >";
}
_9c4.append(arr);
...
dd.push("<div id=\""+(_a26.itemIdPrefix+"_"+i)+"\" class=\""+cls+"\" title='"+s+"' >");//TODO 加上title
